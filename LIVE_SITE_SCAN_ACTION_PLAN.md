...
| Decide AI provider | env | Default: **Google Gemini 2.0 Flash** via its OpenAI‑compatible endpoint (`https://generativelanguage.googleapis.com/v1beta/openai/`). Free tier at aistudio.google.com, no billing. Provider must be swappable behind an interface (see §3). HTML‑only input in MVP (no vision — keeps §5's no‑Puppeteer decision intact). |
| Add env keys | `.env.local` | `GEMINI_API_KEY`, `SCAN_AI_MODEL` (default `gemini-2.0-flash`), `SCAN_MAX_PAGES` (default 50), `SCAN_TIMEOUT_MS` (default 60_000), `SCAN_RATE_LIMIT_PER_HOUR` (default 3). |
| Install deps | `package.json` | `cheerio`, `openai` (used as a generic OpenAI‑compat client, pointed at Gemini's base URL), `fuse.js`, `p-limit`, `robots-parser`. Skip `node-fetch` (Node 18+ has native `fetch`) and `sitemap-parser` (Cheerio in `xmlMode` parses sitemaps). Skip `eventsource-parser` (that is a **client-side** parser; we already receive `text/event-stream` in the browser and can parse it with `Response.body.getReader()` or the native `EventSource`). |
...
12. `aiAnalyzer.ts` — thin provider interface `analyzePage({ url, html, domSummary }) → DetectedComponent[]`. First impl: **Gemini 2.0 Flash** via the `openai` npm client pointed at Gemini's OpenAI‑compat base URL, with a compact library summary in the system prompt. HTML‑only input (Cheerio‑extracted DOM summary + trimmed HTML) — no screenshots in MVP. Response schema validated with Zod; malformed → treat as empty and fall back to heuristics. AI phase uses its own `p-limit(2)` (separate from the crawler's 4) to stay under Gemini free tier (~15 RPM / ~1500 RPD).
...
| Vision / screenshot input to the LLM | Not in MVP | Requires Puppeteer (see row above). MVP feeds Gemini a Cheerio DOM summary + trimmed HTML; upgrade to vision when Puppeteer lands. |
...
| OpenAI GPT‑4o / Anthropic Claude as day‑1 alt provider | Interface only | Ship Gemini 2.0 Flash first (free, generous quota, OpenAI‑compat); provider abstraction in `aiAnalyzer.ts` lets us add OpenAI/Claude later without touching orchestrator/UI. |
...
| 3 | Phase E | `useAi: true` upgrades matches; heuristics fallback verified by unsetting `GEMINI_API_KEY`. |
...
- Running with `GEMINI_API_KEY` unset succeeds via heuristics-only path.
- Gemini 429/503 responses downgrade silently to heuristics-only without failing the scan.
...
4. **Concurrency budget**: Gemini 2.0 Flash free tier is roughly 15 RPM / 1M TPM / 1500 RPD. Cap AI pass at ~10–15 representative pages (one per detected `pageType`) with `p-limit(2)`; heuristics on the rest.
5. **Data policy**: Gemini free‑tier inputs may be used for model training. Surface this in the "Activate AI‑Powered Estimation" toggle copy; consider defaulting `useAi` to off until the user confirms the scanned site is not confidential.